...
func (g *Generator) parseStruct(name string, structType *ast.StructType) StructInfo {
	...
	for _, field := range structType.Fields.List {
		...
		for _, fieldName := range field.Names {
			...
			// Extract query tag value if available
			queryParam := fieldName.Name // Default to field name
			if field.Tag != nil {
				if tag := field.Tag.Value; tag != "" {
					// Parse the struct tag to extract the "query" tag value
					if queryValue := g.extractQueryTag(tag); queryValue != "" {
...
// extractQueryTag extracts the value from the "query" struct tag
func (g *Generator) extractQueryTag(tag string) string {
	// Remove backticks from the tag
	tag = strings.Trim(tag, "`")

	// Look for query:"value" pattern
...
import (
	...
	{{if eq .FilterType "bob"}}"github.com/stephenafamo/bob"
...
var {{.Name}}ColumnsMap = {{if eq $lib "bob"}}bobops.NewBobFilterMap{{else if eq $lib "boiler"}}boilerops.NewBoilerFilterMap{{end}}(map[string]string{
...
func ({{.ReceiverName}} *{{.Name}}) AddFilters(q {{if eq $lib "bob"}}*[]bob.Mod[*dialect.SelectQuery]{{else if eq $lib "boiler"}}*[]qm.QueryMod{{end}}) error {
	{{if eq $lib "bob"}}var qmods []bob.Mod[*dialect.SelectQuery]
	{{else if eq $lib "boiler"}}
