				...
				for _, spec := range x.Specs {
					if typeSpec, ok := spec.(*ast.TypeSpec); ok {
						if structType, ok := typeSpec.Type.(*ast.StructType); ok {
							...
							hasFilter := g.hasFilterComment(x.Doc)
							if hasFilter {
...
// hasFilterComment checks if the struct has a db:filter comment
func (g *Generator) hasFilterComment(doc *ast.CommentGroup) bool {
	if doc == nil {
		return false
	}
...
const codeTemplate = `// Code generated by dbutils/ops/gen. DO NOT EDIT.
...
func ({{.ReceiverName}} *{{.Name}}) AddFilters(q *[]bob.Mod[*dialect.SelectQuery]) error {
	{{if eq $lib "bob"}}filterer := bobops.BobFilterer{}
	var qmods []bob.Mod[*dialect.SelectQuery]
	{{else if $lib "boiler"}}filterer := boilerops.BoilFilterer{}
